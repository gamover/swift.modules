...
Module.prototype.addRoute = function addRoute (controllerName, actionName, path)
{
    ...
    if (typeof path !== 'string' && !$swiftUtils.type.isArray(path))
        throw new $swiftErrors.TypeError('не удалось добавить маршрут в модуле "' + this._moduleName + '". Недопустимый тип пути (ожидается: "string|array", принято: "' + typeof path + '")');
    if (!path.length)
    ...
    {
        this._routes[controllerName][actionName] = this._routes[controllerName][actionName] || [];
        this._routes[controllerName][actionName].push(path);
    }
    else
            ...
            $swiftUtils.fs.exists(pathToController, function (exists)
            {
                ...
                {if (!controllerRoutes.hasOwnProperty(actionName)) continue;
                    ...
                    (function(actionName, controllerRoutes)
                    {
                        ...
                        {
                            if (!error) error = new $swiftErrors.SystemError('в контроллере "' + controllerName + '" модуля "' + self._moduleName + '" не определен экшен "' + actionName + '"');
                            return;
                        }
