...
Module.prototype.setName = function setName (name)
{
    ...
    //
    if (this._isRun)
        throw new $swiftErrors.SystemError('Модуль уже запущен.');
    if (typeof name !== 'string')
        throw new $swiftErrors.TypeError('Недопустимый тип имени модуля (ожидается: "string", принято: "' + typeof name + '").');
    if (!name.length)
...
Module.prototype.setPathToModule = function setPathToModule (pathToModule)
{
    ...
    if (typeof pathToModule !== 'string')
        throw new $swiftErrors.TypeError('Недопустимый тип пути к директории модуля (ожидается: "string", принято: "' + typeof pathToModule + '").');
    if (!pathToModule.length)
        throw new $swiftErrors.ValueError('Недопустимое значение пути к к директории модуля.');
    if (!$swiftUtils.fs.existsSync(pathToModule) || !$fs.statSync(pathToModule).isDirectory())
        throw new $swiftErrors.SystemError('Директория "' + pathToModule + '" не найдена.');
    //
    // задание путей
    //
...
Module.prototype.setPathToModels = function setPathToModels (pathToModels)
{
    ...
    if (typeof pathToModels !== 'string')
        throw new $swiftErrors.TypeError('Недопустимый тип пути к директории моделей (ожидается: "string", принято: "' + typeof pathToModels + '").');
    if (!pathToModels.length)
        throw new $swiftErrors.ValueError('Недопустимое значение пути к к директории моделей.');
    if (!$swiftUtils.fs.existsSync(pathToModels) || !$fs.statSync(pathToModels).isDirectory())
        throw new $swiftErrors.SystemError('Директория "' + pathToModels + '" не найдена.');
    //
    // задание пути к директории моделей
    //
...
Module.prototype.setPathToViews = function setPathToViews (pathToViews)
{
    ...
    if (typeof pathToViews !== 'string')
        throw new $swiftErrors.TypeError('Недопустимый тип пути к директории видов (ожидается: "string", принято: "' + typeof pathToViews + '").');
    if (!pathToViews.length)
        throw new $swiftErrors.ValueError('Недопустимое значение пути к к директории видов.');
    if (!$swiftUtils.fs.existsSync(pathToViews) || !$fs.statSync(pathToViews).isDirectory())
        throw new $swiftErrors.SystemError('Директория "' + pathToViews + '" не найдена.');
    //
    // задание пути к директории видов
    //
...
Module.prototype.setPathToControllers = function setPathToControllers (pathToControllers)
{
    ...
    if (typeof pathToControllers !== 'string')
        throw new $swiftErrors.TypeError('Недопустимый тип пути к директории контроллеров (ожидается: "string", принято: "' + typeof pathToControllers + '").');
    if (!pathToControllers.length)
        throw new $swiftErrors.ValueError('Недопустимое значение пути к к директории контроллеров.');
    if (!$swiftUtils.fs.existsSync(pathToControllers) || !$fs.statSync(pathToControllers).isDirectory())
        throw new $swiftErrors.SystemError('Директория "' + pathToControllers + '" не найдена.');
    //
    // задание пути к директории контроллеров
    //
...
Module.prototype.run = function run (cb)
{
    ...
    if (this._isRun)
    {
        cb(new $swiftErrors.SystemError('Модуль уже запущен.'));
        return this;
    }
    ...
    for (var controllerName in this._routes)
    {
        ...
        (function (controllerName, routes)
        {
            ...
            $swiftUtils.fs.exists(pathToController, function (exists)
            {
                if (!exists)
                {
                    errors.push(new $swiftErrors.SystemError('Контроллер "' + controllerName + '" не найден.'));
                    loading--;
                    return;
                }
                ...
                for (var actionName in controllerRoutes)
                {
                    ...
                    (function(actionName, controllerRoutes)
                    {
                        ...
                        {
                            errors.push(new $swiftErrors.SystemError('В контроллере "' + controllerName + '" не определен экшен "' + actionName + '"'));
                            return;
                        }
    ...
    (function awaiting ()
    {
        immediate(function ()
        {
            ...
            if (errors.length)
            {
                cb(new $swiftErrors.MultipleError('Ошибки запуска модуля.').setList(errors));
                return;
            }
...
Module.prototype.require = function require (path, params, cb)
{
    ...
    if (!path.length)
        error = new $swiftErrors.ValueError('Недопустимое значение пути к ресурсу.');
